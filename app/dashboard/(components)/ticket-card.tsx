  ...
  return (
    <Link href={_ticket(id)}>
      ...
      >
        ...
        <p className="px-2.5 py-1 rounded-sm text-xs bg-green-100 text-green-700 capitalize font-roboto-mono">{payment_status}</p>

        <div
          className={`absolute bottom-0 left-0 right-0 bg-black/60 backdrop-blur-sm p-3 transition-transform duration-300 ease-in-out ${isHovered ? 'translate-y-0' : 'translate-y-full'
            }`}
        >
          <Button className="w-full px-4 py-2 bg-white/30 backdrop-blur-md text-white rounded-full  border-opacity-40 hover:bg-opacity-30 transition-all duration-200">
            View Details
          </Button>
        </div>
      </div>
